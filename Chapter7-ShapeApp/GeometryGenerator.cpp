...
GeometryGenerator::MeshData GeometryGenerator::CreateCylinder(float bottomRadius, float topRadius, float height, uint32 sliceCount, uint32 stackCount)
{
	...
	float ringStep = (topRadius - bottomRadius) / stackCount;
	...
	for (int i = 0; i < ringCount; i++) {
		float y = i * heightStep - 0.5 * height;
		float r = bottomRadius + i * ringStep;
...
void GeometryGenerator::BuildCylinderTopCap(float bottomRadius, float topRadius, float height, uint32 sliceCount, uint32 stackCount, MeshData& meshData)
{
	...
	for (int i = 0; i <= sliceCount; i++) {
		...
		Vertex vertex;
		vertex.Position = XMFLOAT3(x, 0.5f * height, z);
